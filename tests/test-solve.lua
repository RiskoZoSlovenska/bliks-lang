...
local function Instr(funcName, num, pos, ...)
	return structs.Instruction(funcName, { ... }, num, pos)
end
...
describe("the solver", function()
	...
	it("should solve a valid program", function()
		assert.are.same({
			instructions = {
				Instr("set", 1, 18, Pointer(3, 22), String(5, 24)),
				Instr("max", 2, 34, Pointer(3, 38), Number(3, 40), Retr(VT.number, 3, 1, 42),
		...
		assert.are.same({
			instructions = {
				Instr("tonum", 1, 33, Pointer(3, 39), String("jump", 41)),
				Instr("jump", 2, 47, Name("jump", 52)),
			},
			begin = 1,
			std_labels = { jump = { 3 } },
		...
		assert.are.same({
			instructions = {
				Instr("add", 1, 1, Retr(VT.pointer, 1, 2, 5), Retr(VT.number, 1, 3, 9), Retr(VT.number, 1, 3, 11)),
			},
			begin = 1,
		}, parseSolve('add @@1 < <'))
	end)
	...
	it("should reject instructions without a name", function()
		assert.are.same({nil, Error("expected function name, got a literal", 1)}, {parseSolve('3')})
		assert.are.same({nil, Error("expected function name, got a literal", 10)}, {parseSolve('let a 3; "hi" yes')})
		assert.are.same({nil, Error("expected function name, got a retrieval", 10)}, {parseSolve('let a 3; @a yes')})
		assert.are.same({nil, Error("expected function name, got a back retrieval", 10)}, {parseSolve('let a 3; < yes')})
	end)
